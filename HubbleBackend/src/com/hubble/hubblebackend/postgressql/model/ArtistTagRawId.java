package com.hubble.hubblebackend.postgressql.model;

// Generated Sep 7, 2015 5:12:35 PM by Hibernate Tools 3.4.0.CR1

/**
 * ArtistTagRawId generated by hbm2java
 */
public class ArtistTagRawId implements java.io.Serializable {

	private int artist;
	private int editor;
	private int tag;

	public ArtistTagRawId() {
	}

	public ArtistTagRawId(int artist, int editor, int tag) {
		this.artist = artist;
		this.editor = editor;
		this.tag = tag;
	}

	public int getArtist() {
		return this.artist;
	}

	public void setArtist(int artist) {
		this.artist = artist;
	}

	public int getEditor() {
		return this.editor;
	}

	public void setEditor(int editor) {
		this.editor = editor;
	}

	public int getTag() {
		return this.tag;
	}

	public void setTag(int tag) {
		this.tag = tag;
	}

	public boolean equals(Object other) {
		if ((this == other))
			return true;
		if ((other == null))
			return false;
		if (!(other instanceof ArtistTagRawId))
			return false;
		ArtistTagRawId castOther = (ArtistTagRawId) other;

		return (this.getArtist() == castOther.getArtist())
				&& (this.getEditor() == castOther.getEditor())
				&& (this.getTag() == castOther.getTag());
	}

	public int hashCode() {
		int result = 17;

		result = 37 * result + this.getArtist();
		result = 37 * result + this.getEditor();
		result = 37 * result + this.getTag();
		return result;
	}

}
